...
DELETE_PLAN_PROMPT = "لطفاً ID پلنی که می‌خواهید حذف شود را وارد کنید:"
DELETE_PLAN_CONFIRM = "⚠️ آیا از حذف پلن **{plan_name}** با شناسه `{plan_id}` مطمئن هستید؟ این عمل غیرقابل بازگشت است."
PLAN_DELETED_SUCCESS = "✅ پلن '{plan_name}' با موفقیت حذف شد."
EDIT_PLAN_PROMPT_ID = "لطفاً ID پلنی که می‌خواهید ویرایش شود را وارد کنید:"
EDIT_PLAN_NEW_NAME = "لطفاً نام جدید پلن را وارد کنید:"
EDIT_PLAN_NEW_PRICE = "لطفاً قیمت جدید را به تومان وارد کنید:"
# ... (add prompts for other fields like volume and duration)
EDIT_PLAN_SUCCESS = "✅ پلن '{plan_name}' با موفقیت ویرایش شد."

# --- NEW: Custom Config Name ---
ASK_FOR_CUSTOM_CONFIG_NAME = "✅ پرداخت شما تایید شد! حالا یک نام دلخواه برای کانفیگ خود وارد کنید (اختیاری، برای استفاده از نام پیش‌فرض کلمه `skip` را ارسال کنید):"

# --- NEW: How to Connect ---
HOW_TO_CONNECT_TEXT = """
💡 **راهنمای اتصال به سرویس‌ها**

برای استفاده از سرویس‌ها، به یکی از اپلیکیشن‌های زیر نیاز دارید:

📱 **اندروید:**
- **V2RayNG** (پیشنهادی)
  [دانلود از Google Play](https://play.google.com/store/apps/details?id=com.v2ray.ang)
  [دانلود فایل APK](https://github.com/2dust/v2rayNG/releases)

- **NekoBox**
  [دانلود از Google Play](https://play.google.com/store/apps/details?id=moe.nb4a)

🖥️ **ویندوز:**
- **V2RayN**
  [دانلود از GitHub](https://github.com/2dust/v2rayN/releases)

- **NekoRay**
  [دانلود از GitHub](https://github.com/MatsuriDayo/nekoray/releases)

🍏 **آیفون (iOS):**
- **FoXray** (پیشنهادی - رایگان)
- **V2Box** (رایگان)
- **Streisand** (پولی)

**نحوه استفاده:**
۱. لینک اشتراک (Subscription) که از ربات دریافت کرده‌اید را کپی کنید.
۲. وارد اپلیکیشن مورد نظر شوید.
۳. از بخش `Subscriptions` یا `Profiles`، گزینه‌ی `Add` یا `+` را بزنید و لینک را در آنجا Paste کنید.
۴. پروفایل‌ها را آپدیت (Update) کنید تا لیست سرورها نمایش داده شود.
"""
...
CHANNEL_LOCK_MENU_TEXT = """🔗 **مدیریت قفل کانال**

در این بخش می‌توانید یک کانال تلگرامی را به عنوان پیش‌نیاز استفاده از ربات تعیین کنید. تا زمانی که کاربر در این کانال عضو نباشد، ربات به او پاسخی نخواهد داد.

**وضعیت فعلی:** {status}
"""
CHANNEL_LOCK_SET_PROMPT = "لطفاً آیدی عددی کانال مورد نظر را وارد کنید. ربات باید در آن کانال ادمین باشد.\n\nبرای انصراف، `cancel` را ارسال کنید."
CHANNEL_LOCK_SUCCESS = "✅ قفل کانال با موفقیت روی کانال `{channel_id}` تنظیم شد."
CHANNEL_LOCK_REMOVED = "✅ قفل کانال با موفقیت حذف شد."
CHANNEL_LOCK_INVALID_ID = "❌ آیدی وارد شده نامعتبر است. لطفاً یک آیدی عددی صحیح وارد کنید."

# --- NEW: User Management ---
USER_SEARCH_PROMPT = "لطفاً آیدی عددی کاربر مورد نظر برای جستجو را وارد کنید:"
USER_NOT_FOUND = "کاربری با این آیدی یافت نشد."
USER_SUBSCRIPTIONS_HEADER = "اشتراک‌های یافت شده برای کاربر **{user_display}**:"
PURCHASE_DELETED_SUCCESS = "✅ اشتراک با موفقیت حذف شد."



# --- NEW: Support Management ---
SUPPORT_MANAGEMENT_MENU_TEXT = """📞 مدیریت لینک پشتیبانی

در این بخش می‌توانید لینک پشتیبانی (آیدی ادمین، کانال یا گروه) که برای کاربران نمایش داده می‌شود را تنظیم کنید.

لینک فعلی: {link}
"""
SET_SUPPORT_LINK_PROMPT = "لطفاً لینک کامل کانال یا گروه پشتیبانی را وارد کنید (مثلاً: https://t.me/Alamor_Network):"
SUPPORT_TYPE_SET_SUCCESS = "✅ وضعیت پشتیبانی با موفقیت تغییر کرد."
SUPPORT_LINK_SET_SUCCESS = "✅ لینک پشتیبانی با موفقیت ثبت شد."



# --- مدیریت پروفایل‌ها ---
ADD_PROFILE_PROMPT_NAME = "لطفاً یک نام برای پروفایل جدید وارد کنید (مثال: پروفایل گیمینگ اروپا):"
ADD_PROFILE_PROMPT_PER_GB_PRICE = "قیمت هر گیگابایت (GB) برای این پروفایل را به تومان وارد کنید (مثال: 3500):"
ADD_PROFILE_PROMPT_DURATION = "مدت زمان اعتبار کانفیگ‌های این پروفایل را به روز وارد کنید (مثال: 30):"
ADD_PROFILE_PROMPT_DESCRIPTION = "یک توضیح کوتاه برای پروفایل وارد کنید (اختیاری، برای رد شدن کلمه `skip` را وارد کنید):"
ADD_PROFILE_SUCCESS = "✅ پروفایل '{profile_name}' با موفقیت ساخته شد."
ADD_PROFILE_DUPLICATE_ERROR = "❌ پروفایلی با نام '{profile_name}' از قبل وجود دارد."
ADD_PROFILE_GENERAL_ERROR = "❌ خطایی در ساخت پروفایل رخ داد."

NO_PROFILES_AVAILABLE = "😔 متاسفانه در حال حاضر هیچ پروفایلی برای فروش موجود نیست."
SELECT_PROFILE_PROMPT = "🛍️ لطفاً یکی از پروفایل‌های زیر را انتخاب کنید:"


ENTER_PROFILE_GIGABYTES_PROMPT = "لطفاً مقدار حجم مورد نیاز برای این پروفایل را به **گیگابایت** وارد کنید (مثلاً: 20 یا 10.5):"


ADD_DOMAIN_PROMPT = "لطفاً نام دامنه جدید را وارد کنید (مثال: sub.mydomain.com).\n\n**مهم:** قبل از افزودن، مطمئن شوید این دامنه به IP سرور شما اشاره می‌کند (A Record)."
DOMAIN_ADDED_SUCCESS = "✅ دامنه با موفقیت افزوده شد. لطفاً از طریق منو آن را فعال کنید."
DOMAIN_ACTIVATED_SUCCESS = "✅ دامنه '{domain_name}' به عنوان دامنه فعال اشتراک‌ها تنظیم شد."

# --- NEW: Traffic Status Messages ---
TRAFFIC_INFO_HEADER = "📊 **اطلاعات ترافیک سرویس**"
TRAFFIC_NOT_AVAILABLE = "⚠️ اطلاعات ترافیک در دسترس نیست"
TRAFFIC_REFRESH_SUCCESS = "✅ اطلاعات ترافیک بروزرسانی شد"
TRAFFIC_REFRESH_ERROR = "❌ خطا در بروزرسانی اطلاعات ترافیک"
TRAFFIC_EXPIRED_WARNING = "⚠️ **هشدار:** سرویس منقضی شده است!"
TRAFFIC_VOLUME_EXHAUSTED = "⚠️ **هشدار:** حجم سرویس تمام شده است!"